...
Saída
130'''
X = int(input())  # Quota mensal em megabytes
N = int(input())  # Número de meses

quota_restante = X  # Inicialmente, a quota restante é igual à quota mensal

for _ in range(N):
    M = int(input())  # Megabytes usados no mês
    quota_restante += (X - M)  # Atualiza a quota restante com o excedente do mês atual

print(quota_restante)
